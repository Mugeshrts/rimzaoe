...
class MondayPage extends StatefulWidget {
  ...
  final dynamic data;
  final String day;
  const MondayPage({super.key, required this.mode, required this.day, required this.imei,required this.data});
  @override
                            ...
                            children: <Widget>[
                              Stack(
                                children: [
                                  ...
                                  //   bottom: 0,
                                  //   right: 0,
                                  //   child: vvv[index]  ? Icon(Icons.check, color: Colors.blue, size: 20.0) : aaa[index] ? Icon(Icons.check, color: Colors.white, size: 20.0): Container(),
                                  // ),
                                  // Positioned(
                                  //   bottom: 0,
                                  //   right: 10,
    ...
    MqttService.client.updates!.listen((dynamic t) {
      ...
      if(_topic=='$imei/alarmlist'){
        ...
        if(mode=="normal"){
          Exam narmaldata=all_day.normal;
          if(day=="MON"){
            log("day message : ${narmaldata.mon}");
            log("day message type : ${narmaldata.mon.runtimeType}");
            log("time: $tt  , sound :: $ss");
            List checkData=narmaldata.mon;
            log("check  message is present  :: ${checkData.where((element) => element['time'] == tt && element['sound'] == ss).isNotEmpty}");
            ...
            if(checkData.where((element) => element['time'] == tt && element['sound'] == ss).isNotEmpty){
              if(flag=="Inserted"){
